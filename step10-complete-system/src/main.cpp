...
int main(int argc, char* argv[])
{
    try
    {
        ...
        if(app_config.security.enable_tls)
        {
            ...
            if(!app_config.security.cert_file.empty() && !app_config.security.key_file.empty())
            {
                ...
                if(!cert_file)
                {
                    throw std::runtime_error("Failed to open certificate file: " + app_config.security.cert_file);
                }
                ...
                if(!key_file)
                {
                    throw std::runtime_error("Failed to open key file: " + app_config.security.key_file);
                }
