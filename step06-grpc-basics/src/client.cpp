...
public:
  ...
  bool CreateTenant(const std::string &tenant_id, const std::string &name,
    ...
    if (status.ok()) {
      spdlog::info("Created tenant: {} (ID={})", response.tenant().name(),
                   response.tenant().id());
      return true;
    } else {
  ...
  bool GetTenant(const std::string &tenant_id) {
    ...
    if (status.ok()) {
      ...
      spdlog::info("Tenant: {} ({}) - Plan: {}, Active: {}", tenant.name(),
                   tenant.tenant_id(), tenant.plan(),
  ...
  bool ListTenants() {
    ...
    if (status.ok()) {
      ...
      for (const auto &tenant : response.tenants()) {
        spdlog::info("  - {} ({}) [{}]", tenant.name(), tenant.tenant_id(),
                     tenant.plan());
      }
      return true;
    } else {
...
public:
  ...
  bool GetUser(int64_t user_id) {
    ...
    if (status.ok()) {
      ...
      spdlog::info("User: {} <{}> - Role: {}, Active: {}", user.username(),
                   user.email(), user.role(), user.active() ? "yes" : "no");
      return true;
    } else {
  ...
  bool ListUsers() {
    ...
    if (status.ok()) {
      spdlog::info("Found {} users in tenant {}:", response.users_size(),
                   tenant_id_);
      for (const auto &user : response.users()) {
        spdlog::info("  - {} <{}> [{}]", user.username(), user.email(),
                     user.role());
      }
      return true;
    } else {
