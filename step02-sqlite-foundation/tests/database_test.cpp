...
TEST_CASE("Database can be opened", "[database]") {
  ...
  SECTION("With explicit config") {
    DatabaseConfig config{.path = ":memory:", .enable_foreign_keys = true};
    Database db(config);
    REQUIRE(db.handle() != nullptr);
  }
}
...
TEST_CASE("Prepared statements work", "[statement]") {
  ...
  SECTION("Named binding") {
    auto stmt =
        db.prepare("INSERT INTO test (name, score) VALUES (:name, :score)");
    stmt.bind(":name", "Bob");
...
TEST_CASE("Exceptions are thrown correctly", "[exceptions]") {
  ...
  SECTION("Constraint violation") {
    REQUIRE_THROWS_AS(db.execute("INSERT INTO test (value) VALUES ('unique')"),
                      ConstraintException);
  }

  SECTION("Syntax error") {
    REQUIRE_THROWS_AS(db.execute("INVALID SQL"), DatabaseException);
  }
}
