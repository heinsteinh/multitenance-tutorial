...
void demo_basic_operations(db::Database& database) {
    ...
    auto insert_stmt = database.prepare(
...
void demo_transactions(db::Database& database) {
    ...
    // Successful transaction
    {
        ...
        database.execute("INSERT INTO users (name, email) VALUES ('Charlie', 'charlie@example.com')");
        database.execute("INSERT INTO users (name, email) VALUES ('Diana', 'diana@example.com')");
    ...
    // Rolled back transaction
    {
        ...
        database.execute("INSERT INTO users (name, email) VALUES ('Eve', 'eve@example.com')");
        spdlog::info("Inserted Eve (will be rolled back)");
        
        // Transaction rolls back automatically when tx goes out of scope without commit()
        spdlog::info("Transaction will rollback (no commit called)");
    }
    
    // Verify Eve was not added
    auto count = database.query_single<int>("SELECT COUNT(*) FROM users WHERE name = 'Eve'");
...
void demo_savepoints(db::Database& database) {
    ...
    database.execute("INSERT INTO users (name, email) VALUES ('Frank', 'frank@example.com')");
    
    {
        auto sp = database.savepoint("bulk_insert");
        
        try {
            database.execute("INSERT INTO users (name, email) VALUES ('Grace', 'grace@example.com')");
            // This will fail - duplicate email
            database.execute("INSERT INTO users (name, email) VALUES ('Grace2', 'grace@example.com')");
            sp.release();
    ...
    auto frank_exists = database.query_single<int>("SELECT COUNT(*) FROM users WHERE name = 'Frank'");
    auto grace_exists = database.query_single<int>("SELECT COUNT(*) FROM users WHERE name = 'Grace'");
...
void demo_optional_columns(db::Database& database) {
    ...
    auto stmt = database.prepare("INSERT INTO profiles (user_id, bio, website) VALUES (?, ?, ?)");
    stmt.bind(1, 1);
    ...
    // Query with optional
    auto query = database.prepare("SELECT bio, website FROM profiles WHERE user_id = ?");
    query.bind(1, 1);
...
void demo_utility_functions(db::Database& database) {
    ...
    spdlog::info("Table 'users' exists: {}", database.table_exists("users") ? "yes" : "no");
    spdlog::info("Table 'nonexistent' exists: {}", database.table_exists("nonexistent") ? "yes" : "no");
