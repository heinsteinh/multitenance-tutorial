...
Transaction::Transaction(sqlite3 *db, Type type) : db_(db) {
  std::string sql;
  switch (type) {
  case Type::Deferred:
    sql = "BEGIN DEFERRED";
...
Transaction::Transaction(Transaction &&other) noexcept
    : db_(other.db_), active_(other.active_) {
  other.db_ = nullptr;
  other.active_ = false;
}
...
Savepoint::Savepoint(Savepoint &&other) noexcept
    : db_(other.db_), name_(std::move(other.name_)), active_(other.active_) {
  other.db_ = nullptr;
  other.active_ = false;
}
...
Database::Database(Database &&other) noexcept
    : db_(other.db_), config_(std::move(other.config_)) {
  other.db_ = nullptr;
}
...
int Database::total_changes() const { return sqlite3_total_changes(db_); }
...
std::string Database::sqlite_version() { return sqlite3_libversion(); }
