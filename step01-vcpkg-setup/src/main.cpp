...
int main() {
  ...
  json config = {
      {"application", {{"name", "Multi-Tenant System"}, {"version", "1.0.0"}}},
      {"database",
       {{"type", "sqlite"}, {"pool_size", 10}, {"timeout_ms", 5000}}},
      {"grpc", {{"port", 50051}, {"max_threads", 4}}},
      {"tenants",
       {{"isolation", "database_per_tenant"}, {"max_tenants", 100}}}};
  ...
  // Value with default
  int max_connections =
      config.value("/database/max_connections"_json_pointer, 100);
