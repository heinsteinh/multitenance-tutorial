...
namespace db
{
        ...
        : db_(db)
    {
        ...
        int result       = sqlite3_prepare_v2(db_, sql.data(), static_cast<int>(sql.size()),
                                              &stmt_, &tail);
    ...
    Statement& Statement::bind(int index, std::string_view value)
    {
        ...
        check_bind_result(sqlite3_bind_text(stmt_, index, value.data(),
    ...
    {
        check_bind_result(sqlite3_bind_blob(stmt_, index, blob.data(),
    ...
    int Statement::get_param_index(const std::string& name) const
    {
        ...
        if(index == 0)
        {
            throw DatabaseException(SQLITE_ERROR, "Unknown parameter name: " + name);
        }
        return index;
    }
    ...
    void Statement::check_bind_result(int result, int index)
    {
        ...
        {
            throw_sqlite_error(
                result, "Failed to bind parameter " + std::to_string(index), db_);
        }
    }
