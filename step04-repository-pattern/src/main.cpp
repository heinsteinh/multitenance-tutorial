...
void demo_user_repository(pool::ConnectionPool& pool) {
    ...
    std::vector<User> users = {
        ...
        {.tenant_id = "acme-corp", .username = "charlie", .email = "charlie@acme.com",
         .password_hash = "hash3", .role = "user", .active = false},
...
void demo_permission_repository(pool::ConnectionPool& pool) {
    ...
    std::vector<Permission> perms = {
        ...
        {.tenant_id = "acme-corp", .user_id = user->id, .resource = "users", .action = "read"},
        {.tenant_id = "acme-corp", .user_id = user->id, .resource = "users", .action = "update"},
    ...
    for (const auto& p : user_perms) {
        spdlog::info("  - {}:{} = {}", p.resource, p.action, p.allowed ? "allowed" : "denied");
    }
}
...
int main() {
    ...
    try {
        // Create pool
        pool::ConnectionPool pool(pool::PoolConfig{
            .db_path = "step04_demo.db",
            .min_connections = 2,
            .max_connections = 5
        });
