...
public:
  ...
  std::optional<User> find_by_username(const std::string &tenant_id,
                                       const std::string &username) {
    Specification<User> spec;
    spec.where("tenant_id", "=", tenant_id).where("username", "=", username);
    return find_one(spec);
  }

  std::vector<User> find_by_tenant(const std::string &tenant_id) {
    Specification<User> spec;
    spec.where("tenant_id", "=", tenant_id).order_by("username");
    return find_by(spec);
  }
...
protected:
  std::string get_select_columns() const override {
    return "id, tenant_id, username, email, password_hash, role, active, "
           "created_at, updated_at";
  }

  std::string get_insert_columns() const override {
    return "tenant_id, username, email, password_hash, role, active, "
           "created_at, updated_at";
  }
  ...
  std::string get_update_set_clause() const override {
    return "tenant_id = ?, username = ?, email = ?, password_hash = ?, role = "
           "?, active = ?, updated_at = datetime('now')";
  }

  User map_from_row(db::Statement &stmt) const override {
    return User{.id = stmt.column<int64_t>(0),
  ...
  void bind_insert_values(db::Statement &stmt,
...
public:
  ...
  std::vector<Tenant> find_active() {
    Specification<Tenant> spec;
    spec.where("active", "=", static_cast<int64_t>(1)).order_by("name");
    return find_by(spec);
  }

  std::vector<Tenant> find_by_plan(const std::string &plan) {
    Specification<Tenant> spec;
    spec.where("plan", "=", plan).order_by("name");
    return find_by(spec);
  }
...
protected:
  ...
  std::string get_update_set_clause() const override {
    return "tenant_id = ?, name = ?, plan = ?, active = ?, db_path = ?, "
           "updated_at = datetime('now')";
  }

  Tenant map_from_row(db::Statement &stmt) const override {
    return Tenant{.id = stmt.column<int64_t>(0),
                  .tenant_id = stmt.column<std::string>(1),
  ...
  void bind_insert_values(db::Statement &stmt,
                          const Tenant &tenant) const override {
  ...
  int bind_update_values(db::Statement &stmt,
                         const Tenant &tenant) const override {
    stmt.bind(1, tenant.tenant_id);
    stmt.bind(2, tenant.name);
    stmt.bind(3, tenant.plan);
...
public:
  ...
  std::vector<Permission> find_by_user(const std::string &tenant_id,
                                       int64_t user_id) {
    Specification<Permission> spec;
    spec.where("tenant_id", "=", tenant_id).where("user_id", "=", user_id);
    return find_by(spec);
  }
...
protected:
  ...
  Permission map_from_row(db::Statement &stmt) const override {
    return Permission{.id = stmt.column<int64_t>(0),
                      .tenant_id = stmt.column<std::string>(1),
  ...
  void bind_insert_values(db::Statement &stmt,
                          const Permission &perm) const override {
