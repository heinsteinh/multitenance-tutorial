...
struct WhereClause {
  ...
  std::variant<std::monostate, // For IS NULL / IS NOT NULL
               int64_t, double, std::string, std::vector<int64_t>,
 ...
 *       .limit(10);
 */
template <typename Entity> class Specification {
public:
  Specification() = default;

  // ==================== WHERE Clauses ====================

  Specification &where(const std::string &column, const std::string &op,
                       int64_t value) {
  ...
  Specification &where_in(const std::string &column,
  ...
  Specification &where_like(const std::string &column,
  ...
  Specification &order_by(const std::string &column,
                          SortOrder order = SortOrder::Ascending) {
   ...
   */
  template <typename Func> void bind_values(Func &&binder) const {
    int index = 1;
    for (const auto &clause : where_clauses_) {
      std::visit(
          [&](auto &&val) {
  ...
  const std::vector<WhereClause> &where_clauses() const {
    return where_clauses_;
  }
  const std::vector<OrderByClause> &order_by_clauses() const {
