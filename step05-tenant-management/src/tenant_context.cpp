...
int64_t TenantContext::user_id() { return current_user_id_; }

bool TenantContext::has_context() { return has_context_; }
...
TenantScope::TenantScope(const std::string &tenant_id, int64_t user_id)
    : previous_tenant_id_(
          TenantContext::has_context() ? TenantContext::tenant_id() : ""),
