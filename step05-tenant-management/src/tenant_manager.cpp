...
TenantManager::TenantManager(const TenantManagerConfig &config)
  ...
  pool::PoolConfig system_pool_config{.db_path = config_.system_db_path,
                                      .min_connections = 2,
                                      .max_connections = 10,
                                      .enable_foreign_keys = true,
                                      .enable_wal_mode =
                                          config_.enable_wal_mode};
...
TenantManager::TenantManager(const std::string &system_db_path,
    ...
    : TenantManager(
          TenantManagerConfig{.system_db_path = system_db_path,
                              .tenant_db_directory = tenant_db_directory}) {}

TenantManager::~TenantManager() { close_all_pools(); }
...
pool::ConnectionPool &TenantManager::get_pool(const std::string &tenant_id) {
  ...
  if (!is_tenant_active(tenant_id)) {
    throw std::runtime_error(
        fmt::format("Tenant '{}' not found or inactive", tenant_id));
  }
...
pool::ConnectionPool &TenantManager::get_system_pool() { return *system_pool_; }

std::unique_ptr<pool::ConnectionPool>
TenantManager::create_tenant_pool(const std::string &tenant_id) {
  pool::PoolConfig pool_config{.db_path = get_tenant_db_path(tenant_id),
                               .min_connections = config_.pool_min_connections,
                               .max_connections = config_.pool_max_connections,
                               .enable_foreign_keys =
                                   config_.enable_foreign_keys,
...
void TenantManager::provision_tenant(const repository::Tenant &tenant) {
  ...
  if (fs::exists(db_path)) {
    throw std::runtime_error(
        fmt::format("Tenant database already exists: {}", db_path));
  }

  // Create tenant database
  db::Database tenant_db(
...
void TenantManager::run_tenant_schema(db::Database &db) {
  ...
  db.execute("CREATE INDEX IF NOT EXISTS idx_order_items_order ON "
             "order_items(order_id)");

  spdlog::debug("Tenant schema created");
}

void TenantManager::deprovision_tenant(const std::string &tenant_id,
                                       bool delete_data) {
  spdlog::info("Deprovisioning tenant: {} (delete_data={})", tenant_id,
               delete_data);
  ...
  {
    auto conn = system_pool_->acquire();
    auto stmt = conn->prepare("UPDATE tenants SET active = 0, updated_at = "
                              "datetime('now') WHERE tenant_id = ?");
...
void TenantManager::migrate_all_tenants(const MigrationFunc &migration) {
  ...
  for (const auto &tenant_id : tenant_ids) {
    try {
      ...
      spdlog::error("Migration failed for tenant '{}': {}", tenant_id,
                    e.what());
    }
  }
}
...
std::optional<repository::Tenant>
TenantManager::get_tenant(const std::string &tenant_id) {
  auto conn = system_pool_->acquire();
  auto stmt = conn->prepare(R"(
  ...
  if (stmt.step()) {
    return repository::Tenant{.id = stmt.column<int64_t>(0),
...
std::string
TenantManager::get_tenant_db_path(const std::string &tenant_id) const {
...
void TenantManager::preload_all_pools() {
  ...
  for (const auto &tenant_id : tenant_ids) {
    try {
      ...
      spdlog::error("Failed to preload pool for tenant '{}': {}", tenant_id,
                    e.what());
    }
  }
}
...
TenantManager::ManagerStats TenantManager::stats() const {
  ...
  auto conn = system_pool_->acquire();
  auto result =
      conn->query_single<int>("SELECT COUNT(*) FROM tenants WHERE active = 1");
