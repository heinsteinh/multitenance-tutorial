...
PooledConnection::PooledConnection(std::unique_ptr<db::Database> conn,
                                   ReleaseFunc release)
    : conn_(std::move(conn)), release_func_(std::move(release)) {}
...
ConnectionPool::ConnectionPool(const PoolConfig &config) : config_(config) {
  ...
  spdlog::info("Creating connection pool for '{}' (min={}, max={})",
               config_.db_path, config_.min_connections,
...
ConnectionPool::ConnectionPool(const std::string &db_path,
    ...
    : ConnectionPool(PoolConfig{.db_path = db_path,
                                .min_connections = 1,
...
ConnectionPool::~ConnectionPool() {
  ...
  spdlog::info("Connection pool shutdown complete. Stats: {} created, {} "
               "acquisitions, {} timeouts",
               total_created_.load(), total_acquisitions_.load(),
               timeouts_.load());
}
...
std::unique_ptr<db::Database> ConnectionPool::create_connection() {
  db::DatabaseConfig db_config{.path = config_.db_path,
...
PooledConnection ConnectionPool::acquire() {
  ...
  if (!success) {
    ...
    throw std::runtime_error(fmt::format(
        ...
        config_.acquire_timeout.count(), active_count_.load(),
        config_.max_connections));
  }
...
PoolStats ConnectionPool::stats() const {
  ...
  double avg_time =
      acquisitions > 0 ? total_acquire_time_us_.load() / acquisitions : 0.0;

  return PoolStats{.total_connections = total_created_.load(),
...
bool ConnectionPool::is_healthy() const {
  try {
    ...
    db::DatabaseConfig db_config{
        .path = config_.db_path, .create_if_missing = false, .read_only = true};
...
size_t ConnectionPool::active() const { return active_count_.load(); }
